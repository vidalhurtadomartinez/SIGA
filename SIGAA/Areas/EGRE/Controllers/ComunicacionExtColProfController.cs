...
{
    [Autenticado]
    public class ComunicacionExtColProfController : Controller
    {
        private EgresadosContext db = new EgresadosContext();

        [Permiso(Permiso = RolesPermisos.EGRE_comunicacionExternaColProfesional_puedeVerIndice)]
        public ActionResult Index(string criterio = null)

        {
            var comunicacionesExternas = db.ComExternasColegiosProfesionales.Where(p => p.iEliminado_fl == 1).Include(g => g.Alumno).Include(g => g.Perfil).Include(g => g.Persona).Include(g => g.PresidenteColegioProfesional).ToList();
            var comunicacionesExternasFiltradas = comunicacionesExternas.Where(c => criterio == null ||
                                                                         c.PresidenteColegioProfesional.ColegioProfesional.sNombre_nm.ToLower().Contains(criterio.ToLower()) ||
                                                                         c.PresidenteColegioProfesional.sNombreCompleto_desc.ToLower().Contains(criterio.ToLower()) ||
        ...
        [Permiso(Permiso = RolesPermisos.EGRE_comunicacionExternaColProfesional_puedeVerDetalle)]
        ...
        [Permiso(Permiso = RolesPermisos.EGRE_comunicacionExternaColProfesional_puedeCrearNuevo)]
        public ActionResult Create()
        {
            gatbl_ComunicacionExtColProf ComunicacionExt = new gatbl_ComunicacionExtColProf();
        ...
        public ActionResult Create([Bind(Include = "iComExtColProfesional_id,sGestion_desc,sPeriodo_desc,iNumeracion_num,sTipoCom_fl,dtComunicacionExt_dt,dtSorteo_dt,dtHora_dt,dtDefensa_dt,dtHoraDefensa_dt,sLugar_desc,iPerfil_id,lEstudiante_id,lEntregadoPor_id,iPresidenteColProfesional_id,iEstado_fl,iEliminado_fl,sCreado_by,iConcurrencia_id")] gatbl_ComunicacionExtColProf ComunicacionExt)
        {
            try
            {
                if (!esExamenDeGrado(ComunicacionExt.iPerfil_id))//si no es examen de grado los valres para fecha y hora de sorteo y areaDefensa valores equivalentes a nulos
                {
                ...
                {
                    db.ComExternasColegiosProfesionales.Add(ComunicacionExt);
                    db.SaveChanges();
        ...
        [Permiso(Permiso = RolesPermisos.EGRE_comunicacionExternaColProfesional_puedeEditar)]
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "iComExtColProfesional_id,sGestion_desc,sPeriodo_desc,iNumeracion_num,sTipoCom_fl,dtComunicacionExt_dt,dtSorteo_dt,dtHora_dt,dtDefensa_dt,dtHoraDefensa_dt,sLugar_desc,iPerfil_id,lEstudiante_id,lEntregadoPor_id,iPresidenteColProfesional_id,iEstado_fl,iEliminado_fl,sCreado_by,iConcurrencia_id")] gatbl_ComunicacionExtColProf ComunicacionExt)
        {
            try
            {
        ...
        [Permiso(Permiso = RolesPermisos.EGRE_comunicacionExternaColProfesional_puedeEliminar)]
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            using (var transaccion = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    db.ComExternasColegiosProfesionales.Remove(ComunicacionExt);
                    db.SaveChanges();
        ...
        [Permiso(Permiso = RolesPermisos.EGRE_comunicacionExternaColProfesional_puedeImprimir)]
        public ActionResult ImprimirComunicacionExterna(int? id)
        {
            try
            {
                var res = from ce in db.ComExternasColegiosProfesionales
                          ...
                          join pc in db.PresidentesColegiosProfesionales on ce.iPresidenteColProfesional_id equals pc.iPresidenteColProfesional_id
                          join cp in db.Profesionales on pc.iCProfesional_id equals cp.iCProfesional_id
                          join age in db.Personas on ce.lEntregadoPor_id equals age.agd_codigo
                          where ce.iComExtColProfesional_id == id
                          select new
                          {
                              iComExtColProfesional_id = ce.iComExtColProfesional_id,//cambiar este atributo en el ProcAlmacenado
                              ...
                              presiColegio = pc.sNombreCompleto_desc,
                              ColegioProfesional = cp.sNombre_nm,
                ...
                switch (modalidad)
                {
                    case "EXAMEN DE GRADO":
                        {
                            rd.Load(Path.Combine(Server.MapPath("~/Areas/EGRE/Reportes"), "ComExterna_DefensaFinalColProfEG.rpt"));
                            break;
                        }
                    case "GRADUACION POR EXCELENCIA":
                        {
                            rd.Load(Path.Combine(Server.MapPath("~/Areas/EGRE/Reportes"), "ComExterna_DefensaFinalColProfExcelencia.rpt"));
                            break;
                        }
                    case "PRACTICA EMPRESARIAL":
                        {
                            rd.Load(Path.Combine(Server.MapPath("~/Areas/EGRE/Reportes"), "ComExterna_DefensaFinalColProPG.rpt"));
                            break;
                        }
        ...
        public JsonResult BuscarPorRegistro(string term)
        {
            ...
            var personasAl = (from pr in db.Perfiles
                              ...
                              join ag in db.Personas on a.agd_codigo equals ag.agd_codigo
                              where pr.iEliminado_fl == 1 && pr.sPerfil_fl.ToString() == "Aprobado" //validamos NoEsten Eliminados; No Examen de Grado=02; y el perfil este                                                                       
                                                                                                    //select a.alm_registro).ToList();
        ...
        private int ObtenerNumeracion()
        {
            ...
            var comuExtAnio = db.ComExternasColegiosProfesionales.Where(d => d.dtComunicacionExt_dt.Year == anioActual).ToList();
        ...
        //CARGA VIEWBAGS
        private void CargarViewBags(Tarea tarea, gatbl_ComunicacionExtColProf ComunicComunicacionExt)
        {
            ...
            ViewBag.iPresidenteColProfesional_id = new SelectList(db.PresidentesColegiosProfesionales.Where(s => s.iEliminado_fl == 1), "iPresidenteColProfesional_id", "sNombreCompleto_desc", ComunicComunicacionExt.iPresidenteColProfesional_id);
